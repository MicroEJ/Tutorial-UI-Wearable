...
public class WatchfacePage extends Page {
	...
	private Widget createDigital() {
		...
		/**
		 * Step 3.1: Lay out the widgets in a Canvas.
		 *
		 * This time, we will stick to the design of the designer.
		 *
		 * The widgets to use have already been instantiated but they haven't yet been added to a container.
		 *
		 * 1. Instantiate a new Canvas (ej.widget.container.Canvas).<br>
		 * 2. Add the 5 widgets to the canvas using the addChild() method of the canvas. Use the bounds specified for
		 * each widget (x, y, width, height).<br>
		 * 3. Return the canvas.
		 */
	...
	public void populateStylesheet(CascadingStylesheet stylesheet) {
		/**
		 * Step 3.2: Change the style of the digital watchface.
		 *
		 * Change the existing styles accordingly to match with the design (text colors, background image).<br>
		 */
