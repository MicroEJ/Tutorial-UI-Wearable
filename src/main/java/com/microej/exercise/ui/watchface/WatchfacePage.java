...
public class WatchfacePage extends Page {
	...
	public Widget getWidget() {
		...
		/**
		 * STEP 10: Add swipe navigation between the digital watchface and the analog watchface.
		 *
		 * Objective: Learn how to add transitions when navigating between views.
		 *
		 * The SwipeContainer is a container that provides horizontal swipe navigation between its children. We will use
		 * it for this step.
		 *
		 * 1. Instantiate a new SwipeContainer.<br>
		 * 2. Add the already existing digital watchface widget to the swipe container.<br>
		 * 3. Create the analog watchface widget (see createAnalog()).<br>
		 * 4. Add it to the swipe container.<br>
		 * 5. Return the swipe container.
		 */
	...
	private Widget createDigital() {
		...
		canvas.addClassSelector(ClassIdentifiers.DIGITAL_WATCHFACE);
	...
	public void populateStylesheet(CascadingStylesheet stylesheet) {
		...
		// defines the style of the analog watch hands
		style = stylesheet.getSelectorStyle(new ClassSelector(ClassIdentifiers.ANALOG_WATCHFACE));
		style.setBackground(new ImageBackground(Image.getImage(Images.ANALOG_WATCHFACE_BACKGROUND)));
