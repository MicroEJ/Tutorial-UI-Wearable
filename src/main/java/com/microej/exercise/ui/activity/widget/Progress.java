...
public class Progress extends Widget implements MotionAnimationListener {
	...
	public boolean handleEvent(int event) {
		...
		if (type == Pointer.EVENT_TYPE) {
			 ...
			 * progress).
			 *
			 * Objective: Learn how to handle events, to interact with widgets using touch or physical buttons.
			 *
			 * The event is an integer that encodes its type (e.g., pointer) and data (e.g., pointer action). The type
			 * and action have already been decoded for the purposes of the exercise (see above).
			 *
			 * 1. Test the action value of the event.<br>
			 * 2. When the action is Buttons.RELEASED, toggle the value of the boolean showValue.<br>
			 * 3. Request a new render to update the display.<br>
			 * 4. Return true to indicate that the event has been consumed by this widget.<br>
			 */
		}
