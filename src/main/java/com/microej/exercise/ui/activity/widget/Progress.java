...
public class Progress extends Widget implements MotionAnimationListener {
	...
	public void setValue(int newValue) {
		...
		/**
		 * STEP 9: Animate the widget when the value changes.
		 *
		 * Objective: Learn how to use the animation API to make the user interface fancier.
		 *
		 * Step 9.1: Create and start the animation.
		 *
		 * 1. Create a new Motion instance (ej.motion.Motion): specify the easing function, the start value, the stop
		 * value and the duration for the motion.<br>
		 * 2. Create a new MotionAnimation instance (ej.widget.util.motion.MotionAnimation) to bind the motion with this
		 * widget: use the field `animator` and the motion instance as arguments. The widget implements
		 * MotionAnimationListener and will be notified when the motion value changes during the animation.<br>
		 * 3. Call the method startAnimation() to start the motion animation.<br>
		 * 4. Implement the tick() method below to update the widget value when the motion value changes (see the method
		 * thick() below).
		 *
		 */
	...
	public void tick(int value, boolean finished) {
		/**
		 * Step 9.2: Use the value generated by the motion to update the widget content.
		 *
		 * This method is called at each step (i.e., tick) of the animation. The actual update of the rendering should
		 * be done here.
		 *
		 * 1. Set the argument value as the new value for the widget.<br>
		 * 2. Request a new render.
		 */
