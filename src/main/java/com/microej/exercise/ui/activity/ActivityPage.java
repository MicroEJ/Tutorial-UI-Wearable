...
public class ActivityPage extends Page {
	...
	public void update() {
		/**
		 * STEP 7: Update the activity progress widget when the step count value changes.
		 *
		 * Objective: Learn how to update the state/content of a widget and request it to render again.
		 *
		 * The step count updates every 5 seconds but currently the widget does not update:
		 *
		 * 1. Set the new step count value on the activity progress widget (see Progress.setValue()).<br>
		 * 2. Request a new render of the activity progress widget to update the display.
		 *
		 */
